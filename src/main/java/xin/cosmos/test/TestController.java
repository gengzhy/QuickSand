...
public class TestController {
    ...
    public ResultVO<?> handleOcrMerchant(@ApiParam(name = "file", value = "商户证件Excel数据", required = true) @RequestPart(value = "file") MultipartFile file) throws IOException {
        ...
        dataList.forEach(data -> {
            ...
            if (StringUtils.isNotBlank(idCardBackImg)) {
                ...
                try {
                    ...
                    data.setExpiredDate(dateFormat.format(idCard.getExpiredDate()));
                } catch (Exception e) {
            ...
            if (StringUtils.isNotBlank(licenceImg)) {
                ...
                try {
                    ...
                    // 没值取forever（长期）
                    if (StringUtils.isEmpty(license.getValidity()) || "无".equals(license.getValidity())) {
                        data.setValidity("forever");
                    } else {
                        data.setValidity(license.getValidity());
                    }
                } catch (Exception e) {
                    log.error("[{}]关联的身份证[{}]OCR数据处理失败", data.getCode(), licenceImgUrl, e);
                }
            }
            // OCR接口的QPS为10
            try {
        ...
        log.info("OCR处理完毕，共处理数据{}条", dataList.size());
        String filePath = "f:/商户资质补录数据"+System.nanoTime()+".xlsx";
